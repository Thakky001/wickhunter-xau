...
│   ├── style.css             # ไฟล์ CSS จัดรูปแบบ
│   └── app.js                # โค้ดส่วน Frontend (SSE, Render)
├── services/
...
**ตรรกะการคัดกรองสัญญาณ (SMC Entry Logic)**

บอทมี 4 ด่านในการคัดกรองสัญญาณเข้าเทรดที่เข้มงวด:
1. **โซน H1 (Zone):** ราคาย่อลงมาทดสอบ FVG หรือ Order Block บน H1
2. **เทรนด์ H4 (Trend):** ทิศทางต้องสอดคล้องกับเทรนด์ใหญ่ H4 (เช่น H4 ขาขึ้น บอทจะหาเฉพาะจุด BUY เท่านั้น)
3. **แท่งเทียน M5 (Price Action):** ต้องฟอร์มตัวเป็น Hammer (สำหรับ BUY) หรือ Pinbar (สำหรับ SELL) ที่ชัดเจนในโซน
4. **คอนเฟิร์มโครงสร้าง (IDM / ChoCh):** ต้องพบการกวาดสภาพคล่องของรายย่อย (Liquidity Sweep/IDM) หรือ มีการเสียทรงของโครงสร้าง (ChoCh) ก่อนหน้า เพื่อยืนยันว่ารายใหญ่เอาแน่
...
> **การปิดกำไรด้วยตัวเอง (Manual Close):** หากบอทอยู่ในสถานะ `MONITORING_TRADE` และคุณได้ทำการปิดกำไรไปแล้วในแอป MT5 ให้คุณไปที่หน้าเว็บ Dashboard แล้วกดปุ่ม **"🔄 รีเซ็ตออเดอร์"** ที่มุมขวาบน บอทจะทำการเคลียร์ออเดอร์เก่าทิ้งและเปลี่ยนสถานะกลับไปเป็น `SCANNING` เพื่อเริ่มหาไม้ใหม่ให้ทันที
